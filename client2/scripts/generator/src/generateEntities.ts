...
class EntitiesGenerator {
    ...
    generateClass = (sName: string) => {
        ...
        if ($ref) {
            ...
            const importShemaName = `${temp[temp.length - 1]}`;
            entityFile.addImportDeclaration({
                defaultImport: importShemaName,
                moduleSpecifier: `./${importShemaName}`,
                namedImports: [`I${importShemaName}`],
            });

            entityFile.addTypeAlias({
                name: `I${sName}`,
                type: `I${importShemaName}`,
                isExported: true,
            })

            entityFile.addStatements(`export default ${importShemaName};`);
            this.entities.push(entityFile);
            return;
        }
